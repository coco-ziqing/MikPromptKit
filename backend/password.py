...
为何自实现？
- 避免 pip install bcrypt（需编译 C 扩展，Windows 用户零门槛）
...
# P-array 和 S-boxes (from Bruce Schneier's Blowfish)
_P = [
...
# Base64 编码表 (bcrypt 专用，不同于标准 base64)
...
def _bcrypt_b64_encode(data: bytes) -> str:
    ...
    while i < len(data):
        c1 = data[i]
        i += 1
        result += _BCRYPT_BASE64[(c1 >> 2) & 0x3f]
        c1 = (c1 & 0x03) << 4
        if i < len(data):
            c2 = data[i]
            i += 1
            c1 |= (c2 >> 4) & 0x0f
            result += _BCRYPT_BASE64[c1 & 0x3f]
            c1 = (c2 & 0x0f) << 2
            if i < len(data):
                c2 = data[i]
                i += 1
...
def _eks_blowfish_setup(cost: int, salt: bytes, key: bytes):
    ...
    def _encrypt(lr, s):
        ...
        for i in range(16):
            l ^= s[0][i]
            # Feistel network
            r ^= ((s[1][(l >> 24) & 0xff] + s[1][0x100 | ((l >> 16) & 0xff)]) ^ s[1][0x200 | ((l >> 8) & 0xff)]) + s[1][0x300 | (l & 0xff)]
            l, r = r, l
        l, r = r, l
        r ^= s[0][16]
        l ^= s[0][17]
    ...
    result = hashlib.pbkdf2_hmac('sha256', key, salt, 2 ** cost, dklen=24)
    return result


def _generate_salt(rounds: int = 12) -> str:
    """生成随机盐"""
    salt_bytes = os.urandom(16)
    salt = _bcrypt_b64_encode(salt_bytes)[:22]
    return f"$2b${rounds:02d}${salt}"
...
def hash_pw(password: str) -> str:
    """快捷哈希密码 (12轮)"""
    return hash_password(password, 12)
