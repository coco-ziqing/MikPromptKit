...
def rollback_card(card_id: int, data: dict):
    ...
    fields = ["name", "content", "meaning", "scene", "module", "category", "tags", "icon", "group_id", "sort_order", "card_role", "structured", "content_video", "content_video_simple", "content_video_detailed"]
    sets = []
    params = []
    for k in fields:
...
def update_card(card_id: int, data: dict):
    ...
    _save_version_snapshot(db, card_id)
    fields = []; params = []
    for k in ["name","content","meaning","scene","module","category","icon","thumbnail","preview_media","media_type","group_id","sort_order","card_role","content_simple","content_detailed","content_simple_en","content_simple_zh","content_detailed_en","content_detailed_zh","content_zh","content_en","content_video","content_video_simple","content_video_detailed"]:
        if data.get(k) is not None: fields.append(f"{k}=?"); params.append(data[k])
