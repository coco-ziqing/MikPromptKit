...
TIER_SIZES = {
    "hd": (1200, 800),  # 1 列（缩略图封顶 800px 渲染）
    "md": (960, 640),   # 2-3 列（封顶 640px）
    "sd": (600, 400),   # 4-6 列（渲染 ~400/320/265px）
}
...
def serve_video(filename: str):
    ...
    if not os.path.exists(fpath):
        ...
        wc_video_path = os.path.join(_root, "data", "wc_media", "videos", safe_name)
        if os.path.exists(wc_video_path):
            fpath = wc_video_path
        else:
            # v5.37.0: 词卡生成产物视频目录
            cg_video_path = os.path.join(_root, "data", "card_gen", "videos", safe_name)
            if os.path.exists(cg_video_path):
                fpath = cg_video_path
            else:
                # v5.46.10: 即梦资产视频（多版本条视频项 / 视频池切换来源）
                da_video_path = os.path.join(_root, "data", "dreamina_assets", "videos", safe_name)
