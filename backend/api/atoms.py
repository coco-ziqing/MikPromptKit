...
import hashlib, json, asyncio, time
...
async def call_ollama(function: str, prompt: str, system: str = "", temperature: float = 0.3,
                     max_tokens: int = 600, image_base64: str = "") -> str:
    """统一 LLM 调用适配器"""
...
async def decompose(req: DecomposeReq):
    ...
    # LLM 拆解
    prompt = f"[媒体类型:{req.media_type}]\n提示词文本: {req.prompt}"
    try:
        raw = await call_ollama("optimize_fast", prompt, system=DECOMPOSE_SYS, temperature=0.3, max_tokens=600)
        text = str(raw)
