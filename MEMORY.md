...
<!-- openclaw-memory-promotion:memory:memory/2026-05-30.md:107:134 -->
- ## 2026-05-30 23:07 — 导出优化需求初步分析 ### 已检查的代码 - `backend/exporter.py` — PNG 导出引擎（Pillow 绘制卡片 + tEXt 元数据），`import_prompt_from_png` 完备 - `backend/api/exporter.py` — 5 endpoints（单条导出、批量导出 ZIP、预览、单条导入、批量导入）+ JSON 导入 - 前端 `frontend/index.html` — `modalImportExport` 含导入/导出/格式切换 - 前端 `frontend/static/js/app.js` — `renderSidebar` 底部有「导入」「导出」按钮 ### 用户需求分析 1. **不同分类批量导出为可重新识别导入的压缩文件** —— 已有 ZIP 批量导出+导入，但缺少按模块筛选导出 2. **单条 PNG 拖拽导入** —— 已有 PNG 导出+导入（通过弹窗选择文件），但缺少拖拽 PNG 到列表区域自动识别 3. **优化建议** —— 需整体评估 ### 待完成 - 下轮：定位 `showImportModal`/`showExportModal`/`doImport`/`doExport` 方法，实施拖拽导入 + 按模块批量导出 import sys; sys.stdout.reconfigure(encoding='utf-8') fn = r'C:\Users\ASUS\.openclaw\workspace\prompt-tool-dev\frontend\static\js\app.js' with open(fn, encoding='utf-8') as f: c = f.read() # Find import/export related methods near the end i = c.rfind('showImportModal') print('=== showImportModal ===') print(c[i:i+800]) print() i2 = c.rfind('showExportModal') print('=== showExportModal ===') print(c[i2:i2+800]) print() [score=0.926 recalls=4 avg=1.000 source=memory/2026-05-30.md:107-134]
<!-- openclaw-memory-promotion:memory:memory/2026-05-30.md:1:29 -->
- ## 2026-05-30 12:20 — 惰性 seek 优化确认 用户确认： > **确认效果已实现** 优化内容： - 惰性 seek + 单次在途机制消除快速拖拽的 seek 堆积 - 中间值丢弃，只 seek 最新目标位置 - 任意时刻最多 1 个 seek 在处理 当前版本：v3.0.0 运行端口：8080 ## 2026-05-30 23:07 — 侧栏自定义模块 + 弹窗模块下拉修复 + 导出优化需求 ### 完成功能 - [x] **自定义提示词分组**：`custom_modules` 表 + `POST/DELETE /api/modules` 端点 + 侧栏编辑模式下 `➕/✕` 按钮 - [x] **新建词条自动填充当前模块**：`openAddPromptModal` 中 `editModule.value` 设为 `this.state.currentModule` - [x] **模块下拉动态填充**：新增 `_populateModuleOptions()` 用 `/api/modules` 数据动态生成 `<option>`，`openAddPromptModal` 和 `openEditModal` 均调用 - [x] **移除"自定义"兜底 option**：用户确认无用，已删除 ### 待实现（新需求） - [ ] **拖拽 PNG 导入**：文件系统拖拽 PNG 图片到提示词列表区域，自动识别 tEXt 元数据并创建新词条 - [ ] **导出优化建议**：用户需整体评估导出/导入功能的优化方向，包括不同分类批量导出为可重新导入的压缩文件（已有 ZIP 支持）、以及单条 PNG 导出后可拖拽识别 ### 技术状态 - 版本：v3.0.0.1 - 端口：8080 [score=0.873 recalls=3 avg=1.000 source=memory/2026-05-30.md:1-29]
<!-- openclaw-memory-promotion:memory:memory/2026-06-25.md:15:39 -->
- | 7 | 词卡写入/删除 `database is locked` | `safe_commit()` 遇锁直接失败，无重试 | `database.py` | — | | 8 | 中英文翻译 500 | `translate_single` 持DB连接等Ollama(5-30s)，全程阻塞其他写入 | `translate.py` | — | ## 关键技术决策 ### DB 锁问题三类根因 → 统一修复模式 1. **健康检测** → 独立 `sqlite3.connect`（不共用 `get_db()`） 2. **safe_commit** → 3次重试 + 递增等待(50ms/150ms/400ms) 3. **翻译端点** → 三段短连接（读→close→Ollama→独立连接写缓存） ### 猴补丁链缺陷（长期风险） 4 个 composer 模块各自猴补丁 `switchView`，链式嵌套： `atom_editor → scene_composer → unified_composer → character_composer → 原始 app_core.js` **缓解**：`app_core.js` 新增统一 `else if` 分支做 `_hideSearchBox + _showSidebar + _collapseSidebar + 导航高亮`，猴补丁变为增强非唯一通道。 ## 验证状态 - 健康自检: 10/10 ✅ - 核心 API 19端点: 全部 200 ✅ - E2E 流程 (场景/角色/词卡/翻译): 5/5 通过 ✅ [score=0.871 recalls=3 avg=1.000 source=memory/2026-06-25.md:15-35]
<!-- openclaw-memory-promotion:memory:memory/2026-07-02.md:23:23 -->
- **后端** `backend/api/v2.py` SQL 补齐 4 个缺失字段： [score=0.856 recalls=0 avg=0.620 source=memory/2026-07-02.md:23-23]
<!-- openclaw-memory-promotion:memory:memory/2026-07-02.md:29:29 -->
- **前端** `app_collections.js`： [score=0.856 recalls=0 avg=0.620 source=memory/2026-07-02.md:29-29]
