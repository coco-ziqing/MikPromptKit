...
App._showActivationDialog = function(mode, tier) {
    ...
    ov.innerHTML = '<div class="modal-box" style="max-width:440px;background:var(--bg-card);border-radius:12px;padding:24px;" onclick="event.stopPropagation()">' +
        ...
        '<div style="font-size:11px;color:var(--text-muted);margin-bottom:12px;padding:8px;background:var(--bg);border-radius:6px;">本机指纹: <code style="word-break:break-all;">' + (App._licenseFingerprint||'加载中...') + '</code> <span onclick="var fp=(App._licenseFingerprint||\x27\x27);navigator.clipboard.writeText(fp).then(function(){var t=document.getElementById(\x27licFpCopy\x27);t.textContent=\x27\u2705 已复制\x27;setTimeout(function(){t.textContent=\x27\uD83D\uDCCB\x27},1800)}).catch(function(){var ta=document.createElement(\x27textarea\x27);ta.value=fp;ta.style.position=\x27fixed\x27;ta.style.left=\x27-9999px\x27;document.body.appendChild(ta);ta.select();document.execCommand(\x27copy\x27);document.body.removeChild(ta);var t=document.getElementById(\x27licFpCopy\x27);t.textContent=\x27\u2705 已复制\x27;setTimeout(function(){t.textContent=\x27\uD83D\uDCCB\x27},1800)})" id="licFpCopy" title="复制指纹" style="cursor:pointer;margin-left:6px;font-size:14px;user-select:none;">\uD83D\uDCCB</span> <a href="/static/keygen.html" target="_blank" style="color:var(--primary);font-weight:600;">\uD83D\uDD0C 生成器</a></div>' +
