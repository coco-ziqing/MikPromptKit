                ...
                <div class="prompt-card ${batchClass} ${selectedClass} ${editClass}" data-id="${p.id}">
                    <div class="card-body">
                        <div class="card-thumb">
                            <div class="card-thumb-inner" onclick="${isWordCard ? 'if(App.state.editMode)App.wordEditor.open({cardId:' + p.id + ',source:\'cards\',onSaved:function(){App.loadPrompts()}})' : 'if(App.state.editMode)App.showThumbnailPicker(' + p.id + ')'}">
                                ${videoFile
                            ...
                            ${(p.thumbnail || videoFile) ? '<span class="thumb-zoom-btn" onclick="event.stopPropagation();' + (videoFile ? 'App.openVideoViewer(\'' + videoFile + '\', \'' + (p.thumbnail || '') + '\', \'' + p.id + '\', \'' + videoFps + '\')' : 'App.openImageViewer(\'' + origFile + '\', \'' + p.id + '\')') + '" title="' + (videoFile ? App._t('auto.view_原视频', '查看原视频') : App._t('auto.view_原图', '查看原图')) + '">' + (videoFile ? '▶' : '🔍') + '</span>' : ''}
                        </div>
