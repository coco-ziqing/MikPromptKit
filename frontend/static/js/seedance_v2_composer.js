...
(function() {
    ...
    App.seedanceV2.renderProjectEditor = function() {
        ...
        // ① 分镜列表
        h+='<div class="s2-section"><div class="s2-section-title" style="cursor:pointer;">🎬 分镜列表 <span class="s2-badge">'+this.scenes.length+' 镜头</span></div><div class="s2-timeline-wrapper"><div class="s2-timeline-ticks">';var tickSpan=Math.max(1,Math.floor((p.total_duration||15)/6));for(var tk=0;tk<=p.total_duration;tk+=tickSpan){h+='<span>'+tk+'s</span>';}h+='</div><div class="s2-timeline-bar" id="s2TimelineBar">';for(var i=0;i<this.scenes.length;i++){var s=this.scenes[i];var w=Math.max(3,((s.end_time-s.start_time)/(p.total_duration||15))*100);var lb=(s.subject||'镜头'+(i+1)).substring(0,6);h+='<div class="s2-timeline-seg" data-scene-id="'+s.id+'" style="width:'+w+'%;" title="'+s.start_time+'-'+s.end_time+'s: '+App._escape(lb)+'" onclick="App.seedanceV2._scrollToScene('+s.id+')"><span>'+lb+'</span></div>';}h+='</div></div><div class="s2-scenes-container" id="s2ScenesContainer"></div></div>';
        // ② 全局参数（分镜设完再调全局）
